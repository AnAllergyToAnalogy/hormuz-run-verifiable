...
use crate::error::*;
use crate::constants::{DEMAND_PRECISION};

pub fn _get_demand<'info>(
        timestamp: u64,
        game: &mut Account<'info, Game>, 
    ) -> u64 {

    let multiplier = game.demand_multiplier as u64;

    if timestamp > game.demand_end_time {
        return multiplier * DEMAND_PRECISION ;
    }

    return DEMAND_PRECISION + DEMAND_PRECISION * (multiplier - 1) * (timestamp - game.demand_start_time) / game.demand_duration;
